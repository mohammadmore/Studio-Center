...
app.use("/api/login.php", loginLimiter);
...
app.use("/api/upload_image.php", uploadLimiter);
app.use("/api/bulk_import_contents.php", uploadLimiter);
app.use("/api/upload_smart_events.php", uploadLimiter);
app.use("/api/upload_font.php", uploadLimiter);
...
*, body, button, input, select, textarea, .asva-menu-link, .btn-primary, .btn-outline, .form-control, .s-tab, .tab-btn {
  ...
  res.on("finish", () => {
    if (req.method === "POST") {
      if (req.path.startsWith("/api/") && !req.path.includes("auth.php")) {
        const action = req.body?.action || req.query?.action || "POST";
        let endpoint = req.path.replace("/api/", "").replace(".php", "");
        const epNames = {
          manage_team:
            "\u0645\u062F\u06CC\u0631\u06CC\u062A \u06A9\u0627\u0631\u0628\u0631\u0627\u0646/\u0647\u0645\u06A9\u0627\u0631\u0627\u0646",
        ...
        if (action === "edit" || action === "update")
          actionFa = "\u0648\u06CC\u0631\u0627\u06CC\u0634";
        if (action === "delete") actionFa = "\u062D\u0630\u0641";
        if (action === "batch_delete")
          actionFa = "\u062D\u0630\u0641 \u06AF\u0631\u0648\u0647\u06CC";
        if (action === "update_status")
          actionFa =
            "\u062A\u063A\u06CC\u06CC\u0631 \u0648\u0636\u0639\u06CC\u062A";
        if (action === "approve") actionFa = "\u062A\u0627\u06CC\u06CC\u062F";
        if (action === "reject") actionFa = "\u0631\u062F";
        if (action === "POST")
          actionFa =
            "\u0630\u062E\u06CC\u0631\u0647/\u0622\u067E\u0644\u0648\u062F";
        let epFa = epNames[endpoint] || endpoint;
        let finalDesc = `${actionFa} \u062F\u0631 ${epFa}`;
...
const authGuard = __name((req, res, next) => {
  ...
  if (session2 && session2.user) {
    if (session2.user?.role_id === "client") {
      return res.redirect("/client-dashboard.php");
    }
...
app.get("/", (req, res) => {
  ...
  if (req.session && req.session.user) {
    return res.redirect("/depot.php");
  }
  res.render("index", { current: "index.php" });
});
...
app.get("/depot.php", authGuard, (req, res) => {
  res.render("depot", { current: "depot.php" });
});
app.get("/collections.php", authGuard, (req, res) => {
  res.render("collections", { current: "collections.php" });
});
app.get("/archive.php", authGuard, (req, res) => {
  res.render("archive", { current: "archive.php" });
});
app.get("/my-projects.php", authGuard, (req, res) => {
  res.render("my-projects", { current: "my-projects.php" });
});
app.get("/dashboard.php", authGuard, (req, res) => {
  res.render("dashboard", { current: "dashboard.php" });
});
app.get("/calendar.php", authGuard, (req, res) => {
  res.render("calendar", { current: "calendar.php" });
});
app.get("/smart-events.php", authGuard, (req, res) => {
  res.render("smart-events", { current: "smart-events.php" });
});
app.get("/client-events.php", clientAuthGuard, (req, res) => {
  const orgId = req.query.org_id ? parseInt(req.query.org_id) : null;
  const org = organizations.find((o) => o.id === orgId) || null;
  res.render("client-events", { org });
});
app.get("/client-review.php", clientAuthGuard, (req, res) => {
  const orgId = req.query.org_id ? parseInt(req.query.org_id) : null;
  const org = organizations.find((o) => o.id === orgId) || null;
  res.render("client-review", { org });
});
app.get("/media.php", authGuard, (req, res) => {
  res.render("media", { current: "media.php" });
});
app.get("/templates.php", authGuard, (req, res) => {
  res.render("templates", { current: "templates.php" });
});
app.get("/organizations.php", authGuard, (req, res) => {
  res.render("organizations", { current: "organizations.php" });
});
app.get("/client-portal.php", authGuard, (req, res) => {
  res.render("client-portal", { current: "client-portal.php" });
});
app.get("/client-dashboard.php", clientAuthGuard, (req, res) => {
  const session2 = req.session;
...
app.get("/invoices_list.php", authGuard, (req, res) => {
  res.render("invoices_list", { current: "invoices_list.php" });
});
app.get("/invoice.php", authGuard, (req, res) => {
  res.render("invoice", { current: "invoice.php" });
});
app.get("/reports.php", (req, res) => {
  const session2 = req.session;
  ...
  if (isClient) {
    ...
    return res.render("reports", { current: "reports.php", isClient: true });
  } else {
    const userRole = roles.find((r) => r.id === session2.user?.role_id);
    res.locals.userPermissions = userRole ? userRole.permissions : [];
    res.locals.user = session2.user;
    return res.render("reports", { current: "reports.php", isClient: false });
  }
});
app.get("/finance.php", authGuard, (req, res) => {
  res.render("finance", { current: "finance.php" });
});
app.get("/add-user.php", authGuard, (req, res) => {
  res.render("add-user", { current: "add-user.php" });
});
app.get("/leaves.php", authGuard, (req, res) => {
  res.render("leaves", { current: "leaves.php" });
});
app.get("/team.php", authGuard, (req, res) => {
  res.render("team", { current: "team.php" });
});
app.get("/settings.php", authGuard, (req, res) => {
  res.render("settings", {
    current: "settings.php",
    permissionCategories: getPermissionCategories(),
  });
});
app.get("/audit.php", authGuard, (req, res) => {
  res.render("audit", { current: "audit.php" });
});
app.get("/recycle_bin.php", authGuard, (req, res) => {
  res.render("recycle_bin", { current: "recycle_bin.php" });
});
app.get("/tokens.php", authGuard, (req, res) => {
  res.render("tokens", { current: "tokens.php" });
});
app.get("/notifications.php", authGuard, (req, res) => {
  res.render("notifications", { current: "notifications.php" });
});
  ...
  if (session2 && session2.user) {
    if (session2.user?.role_id === "client") {
      const allowedForClients = [
        ...
        "/read_notifications.php",
        "/get_smart_events_monthly.php",
        "/add_client_events.php",
        "/get_client_data.php",
        "/client_review.php",
        "/get_org_report.php",
        "/get_studio_info.php",
        "/download_content_excel_template.php",
      ];
...
apiRouter.get("/logout.php", (req, res) => {
  if (req.session) {
    req.session.destroy(() => {
      res.redirect("/");
    });
...
apiRouter.get("/check_auth.php", (req, res) => {
  const session2 = req.session;
  if (session2 && session2.user) {
    const roleInfo = getRoleInfoForUser(session2.user);
    const colleague = colleagues.find(
...
apiRouter.get("/get_roles.php", (req, res) => {
...
apiRouter.get("/global_search.php", (req, res) => {
...
apiRouter.get("/get_collections.php", (req, res) => {
...
apiRouter.get("/get_depot_contents.php", (req, res) => {
...
apiRouter.get("/get_my_projects.php", (req, res) => {
  const orgMap = new Map();
  organizations.forEach((o) => orgMap.set(o.id.toString(), o));
  req._orgMap = orgMap;
...
apiRouter.get("/get_org_contents.php", (req, res) => {
...
apiRouter.post("/add_content.php", checkPermission("depot"), (req, res) => {
  const {
    action,
    id,
    status,
...
apiRouter.get("/download_content_excel_template.php", (req, res) => {
  try {
...
apiRouter.get("/get_dashboard_stats.php", (req, res) => {
  const bottlenecks = {};
...
apiRouter.get("/get_smart_events_monthly.php", (req, res) => {
...
apiRouter.get("/get_events.php", (req, res) => {
...
apiRouter.get("/manage_settings.php", (req, res) => {
...
apiRouter.get("/get_orgs.php", (req, res) => {
...
apiRouter.get("/manage_users.php", checkPermission("add-user"), (req, res) => {
...
apiRouter.post("/manage_users.php", checkPermission("add-user"), (req, res) => {
  const {
    action,
    id,
    full_name,
...
apiRouter.get("/get_colleagues.php", (req, res) => {
  const mapped = colleagues
...
apiRouter.post("/save_leave.php", (req, res) => {
  const { leave_user, substitute_user, start_date, end_date } = req.body;
  const session2 = req.session;
  ...
  if (
    ...
    return res
      ...
      .json({
        status: "error",
        message:
          "\u0634\u0645\u0627 \u0641\u0642\u0637 \u0645\u06CC\u200C\u062A\u0648\u0627\u0646\u06CC\u062F \u0628\u0631\u0627\u06CC \u062E\u0648\u062F \u0645\u0631\u062E\u0635\u06CC \u062B\u0628\u062A \u06A9\u0646\u06CC\u062F",
      });
  }
  ...
  res.json({
    status: "success",
    message:
      "\u0645\u0631\u062E\u0635\u06CC \u0628\u0627 \u0645\u0648\u0641\u0642\u06CC\u062A \u062B\u0628\u062A \u0648 \u062A\u0633\u06A9\u200C\u0647\u0627 \u0645\u0646\u062A\u0642\u0644 \u0634\u062F",
  });
});
apiRouter.post("/save_user.php", checkPermission("add-user"), (req, res) => {
  const {
    full_name,
    role_id,
    username,
...
apiRouter.post("/upload_image.php", async (req, res) => {
...
apiRouter.post("/upload_file.php", upload.single("file"), async (req, res) => {
  if (!req.file) {
    return res
      .status(400)
      .json({
...
apiRouter.get("/get_logs.php", checkPermission("audit"), (req, res) => {
  res.json({ status: "success", data: audit_logs });
});
apiRouter.get("/get_notifications.php", (req, res) => {
  const session2 = req.session;
  if (!session2 || !session2.user)
    return res.status(401).json({ status: "error" });
...
apiRouter.post("/read_notifications.php", (req, res) => {
  const session2 = req.session;
  if (!session2 || !session2.user)
    return res.status(401).json({ status: "error" });
  const user = session2.user;
...
apiRouter.post("/unlock_events.php", (req, res) => {
  ...
  if (existingSelectionIndex >= 0) {
    ...
    addNotification(
      ...
      "client",
      numericOrgId,
      `/client-events.php?year=${targetYear}&month=${targetMonth}`,
    );
    const jalaaliDate = new Intl.DateTimeFormat("fa-IR", {
      year: "numeric",
      month: "2-digit",
...
apiRouter.post("/save_token.php", checkPermission("settings"), (req, res) => {
...
apiRouter.post("/fetch_live_stats.php", (req, res) => {
...
apiRouter.get("/get_tokens.php", (req, res) => {
  res.json({ status: "success", tokens });
});
apiRouter.get("/get_templates.php", (req, res) => {
  res.json({ status: "success", templates: templatesData });
});
apiRouter.post(
  "/manage_content_template.php",
...
apiRouter.get("/get_content_templates.php", (req, res) => {
  res.json(contentTemplates);
});
apiRouter.post(
  "/apply_content_template.php",
  checkPermission("depot"),
  (req, res) => {
...
apiRouter.post("/dismiss_template_suggestion.php", (req, res) => {
...
apiRouter.post("/fulfill_template_suggestion.php", (req, res) => {
...
apiRouter.post("/apply_template.php", checkPermission("depot"), (req, res) => {
...
apiRouter.get("/get_media.php", (req, res) => {
...
apiRouter.post("/save_media.php", (req, res) => {
...
apiRouter.post("/save_onboarding.php", (req, res) => {
...
apiRouter.post("/save_expense.php", checkPermission("finance"), (req, res) => {
...
apiRouter.get("/get_orgs_public.php", (req, res) => {
  res.json(organizations);
});
apiRouter.get("/get_client_data.php", (req, res) => {
  const org_id = parseInt(req.query.org_id);
...
apiRouter.post("/client_review.php", (req, res) => {
...
apiRouter.post("/confirm_events.php", (req, res) => {
...
apiRouter.get("/get_fonts.php", async (req, res) => {
  let fontDocs = [];
  try {
...
apiRouter.post("/rename_font.php", checkPermission("settings"), (req, res) => {
...
apiRouter.post("/toggle_stage.php", checkPermission("depot"), (req, res) => {
...
apiRouter.get("/invoices.php", checkPermission("invoice"), (req, res) => {
  res.json({ status: "success", data: invoices });
});
apiRouter.post("/invoices.php", checkPermission("invoice"), (req, res) => {
  const {
    action,
    id,
    org_id,
...
apiRouter.get("/get_org_report.php", checkPermission("reports"), (req, res) => {
  const org_id = req.query.org_id;
  const start_date = req.query.start_date;
  const end_date = req.query.end_date;
  const col_id = req.query.col_id;
...
apiRouter.get("/get_studio_info.php", (req, res) => {
  res.json({ status: "success", data: settings.studio_info });
});
apiRouter.post(
  "/save_studio_info.php",
  checkPermission("settings"),
  (req, res) => {
    if (req.body) {
...
apiRouter.get("/get_batch_operations.php", (req, res) => {
...
apiRouter.post("/undo_batch_operation.php", (req, res) => {
  const username =
    req.session && req.session.user && req.session.user.username
      ? req.session.user.username
      : "admin";
